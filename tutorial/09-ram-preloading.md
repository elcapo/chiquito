...
So far, our forward pass loads each layer from disk on every call. For 67 layers x 20 tokens = 1,340 disk reads, this is painfully slow. This unit introduces the three loading strategies that trade RAM for speed, implemented in [`model.py`](../src/chiquito/model.py).
...
The constructor interprets this parameter at [`model.py:129-135`](../src/chiquito/model.py#L129-L135):
...
([`model.py:294-301`](../src/chiquito/model.py#L294-L301))

During the forward pass, `_load_layer_to_cpu()` is just a dictionary lookup ([`model.py:311-313`](../src/chiquito/model.py#L311-L313)):
...
When RAM is extremely limited, weights are read from disk on every layer load. `_load_layer_to_cpu()` falls through to the disk path ([`model.py:316`](../src/chiquito/model.py#L316)):
...
The forward pass sets this up at [`model.py:468-475`](../src/chiquito/model.py#L468-L475):
...
And inside the loop ([`model.py:484-491`](../src/chiquito/model.py#L484-L491)):
...
This is a classic **producer-consumer** pattern implemented in the `_SlidingWindowCache` class ([`model.py:27-94`](../src/chiquito/model.py#L27-L94)).
...
Before the background thread starts, the first N layers are loaded synchronously ([`model.py:47-58`](../src/chiquito/model.py#L47-L58)):
...
([`model.py:60-69`](../src/chiquito/model.py#L60-L69))
...
([`model.py:78-94`](../src/chiquito/model.py#L78-L94))
...
Because `forward()` is called once per token and iterates through all layers, the sliding window cache must be restarted for each forward call ([`model.py:437-438`](../src/chiquito/model.py#L437-L438)):
...
`_start_window_cache()` stops the old cache and creates a fresh one ([`model.py:303-309`](../src/chiquito/model.py#L303-L309)):
...
([`model.py:71-76`](../src/chiquito/model.py#L71-L76))
...
The `_load_layer_to_cpu()` method dispatches based on which cache is active ([`model.py:311-316`](../src/chiquito/model.py#L311-L316)):
